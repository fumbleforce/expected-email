...
export default class Session {
  ...
  async getSession (forceUpdate) {
    ...
    const sessionExists = this._session && Object.keys(this._session).length > 0;
    const sessionNotExpired = sessionExists &&
      this._session.expires &&
      this._session.expires > Date.now();
    if (sessionExists && sessionNotExpired) {
      return new Promise(resolve => {
        resolve(this._session);
      });
    }
